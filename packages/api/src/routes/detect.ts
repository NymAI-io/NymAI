...
import { detect, type DataType, type Finding } from '@nymai/core';
...
function getEnabledTypes(config: WorkspaceConfig | null): DataType[] {
  ...
  if (config.detect_dob) types.push('DOB');
  if (config.detect_passport) types.push('PASSPORT');
  if (config.detect_bank_account) types.push('BANK_ACCOUNT');
  if (config.detect_routing) types.push('ROUTING');
  if (config.detect_ip_address) types.push('IP_ADDRESS');
  if (config.detect_medicare) types.push('MEDICARE');
  if (config.detect_itin) types.push('ITIN');

  return types.length > 0 ? types : ALL_DATA_TYPES;
}
