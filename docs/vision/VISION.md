...
- **Current solution:** GitGuardian, Gitleaks, TruffleHog cover *secrets* well but almost ignore PII entirely; no unified detection.
...
|----------|----------|---------|------------------|
| **DLP (Post-hoc detection)** | Strac, Nightfall, Polymer | Detect *after* data is stored; store customer data for ML training | Nightfall ($60M raised) |
...
1. **Real-time Prevention (vs. Post-hoc Detection)**
   - Every competitor detects *after* data is stored/sent.
...
**Positioning:** We're a *complement* to ADPP, not a replacement. Different architecture, different coverage.

---

## V. Future Optionality: Step 3 Expansion

> **Important:** This section describes *optional* expansion that will only be considered after achieving $30k MRR sustained for 3+ months. This is NOT a committed roadmap.
...
### 4. Core Engine Portability (Future Moat)

- **`@nymai/core`:** Works in Node.js, browsers, edge runtimes.
- **Advantage:** If Step 3 is pursued, same engine powers all surfaces.
