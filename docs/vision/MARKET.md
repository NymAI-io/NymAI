...
> **📖 For detailed competitor analysis, messaging playbooks, and win/loss scenarios, see [COMPETITIVE.md](./COMPETITIVE.md) (internal only).**
...
| ---------------- | --------------------- | -------------------------------------- | ---------------------------------------------------- | ------------------------- |
| **Zendesk ADPP** | Native DLP            | Bundled; zero integration              | Forward-only; 5 patterns; stores detected values     | Included                  |
| **Strac**        | Enterprise DLP        | ML-powered; 100+ file types; YC-backed | Stores originals in vault; enterprise sales required | Custom ($10k+/year)       |
| **Nightfall AI** | Enterprise DLP        | 95%+ precision; broad coverage         | Cloud-first; enterprise-only; stores data for ML     | Custom ($50k+/year)       |
| **Metomic**      | SaaS data discovery   | Wide SaaS coverage                     | Detection-only; no redaction                         | Custom ($11k median/year) |
| **Teleskope.ai** | Data privacy platform | Multiple redaction methods             | No Marketplace presence; enterprise-focused          | Custom                    |

#### Zendesk Marketplace Apps (Direct Competition)

| Competitor                        | What It Does                           | Strengths                           | Weaknesses                                                                         | Pricing  |
| --------------------------------- | -------------------------------------- | ----------------------------------- | ---------------------------------------------------------------------------------- | -------- |
| **Redact Attachments** (Knots.io) | Attachment-only redaction via triggers | Free; automated; execution logs     | Text NOT covered; needs separate app for text; can't redact inline images          | Free     |
| **Redacto**                       | Bulk attachment redaction              | Cheap; local processing; CSV import | No text; no preview; no undo; batch-focused                                        | $9.99/mo |
| **eesel AI**                      | AI assistant with PII feature          | AI-powered; free signup             | PII is secondary feature, not core product; limited Zendesk-specific functionality | Custom   |

#### Open Source / Developer Tools

| Competitor        | What It Does                              | Strengths                                        | Weaknesses                                                 | Pricing |
| ----------------- | ----------------------------------------- | ------------------------------------------------ | ---------------------------------------------------------- | ------- |
| **OpenRedaction** | Regex-based PII redaction (500+ patterns) | MIT licensed; self-hostable; zero data retention | No Zendesk integration; CLI-only; requires technical setup | Free    |
...
### 7.2 Key Differentiators (10x Positioning)

| Differentiator                 | What It Means                           | Proof Point                         | Competitors Lacking This                       |
| ------------------------------ | --------------------------------------- | ----------------------------------- | ---------------------------------------------- |
| **Ephemeral processing**       | Raw PII never stored anywhere           | <500ms in memory, then cleared      | Strac (vault), Nightfall (ML training), eesel  |
| **All-data scanning**          | Works on historical data + attachments  | Scan existing tickets on demand     | ADPP (forward-only), Nightfall                 |
| **Text + Attachments unified** | One app for everything                  | Single sidebar, single workflow     | Redact Attachments, Redacto (attachments only) |
| **Preview + Undo**             | See before you redact; recover mistakes | 10-second undo window               | All competitors lack undo                      |
| **SMB pricing**                | Accessible without enterprise budget    | $499/month starting                 | Strac, Nightfall, Teleskope ($10k+/year)       |
| **Self-serve**                 | No 6-month sales cycle                  | 14-day trial → credit card purchase | Strac, Teleskope (enterprise sales required)   |

#### The 10x Summary

We're the **ONLY** Zendesk DLP option that combines:

- ✅ Text + attachment scanning in **ONE app** (vs. Redact Attachments + AI Ticket Redaction)
- ✅ Public, self-serve pricing at **$499-$899/mo** (vs. Strac/Nightfall at $10k-$50k+/year)
- ✅ **Ephemeral processing** with no data stored (vs. Strac vault, Nightfall ML training)
- ✅ **Agent-facing sidebar** with preview (vs. Redacto batch processing, OpenRedaction CLI)
- ✅ **Undo capability** (vs. everyone else's destructive-only redaction)

> **See [COMPETITIVE.md](./COMPETITIVE.md) for detailed messaging against each competitor.**
...
_Version: 2.0 (Bootstrapped Path)_  
